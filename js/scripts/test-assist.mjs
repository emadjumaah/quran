...
// ——— دورة محادثة ———
/** كما في المتصفح: تنقية الاقتباسات القرآنية ثم طباعة الجواب النهائي */
function finish(rawText, steps, toolTexts) {
  const { text, fixed } = enforceVerbatim(rawText, toolTexts);
...
async function chatTurn(messages, label) {
  ...
  for (let round = 0; round < 5; round++) {
    ...
    if (j.finalize) {
      ...
      return finish(j2.text || j.text || "", steps, toolTexts);
    }
    if (j.text) return finish(j.text, steps, toolTexts);
