...
/** جاراتُ المعنى — من التضمينات المخزّنة في القاعدة (كما يفعل التطبيق بالملفّ) */
let neighborsOf = () => [];
try {
  const rows = db.prepare("SELECT ayah_id, vector FROM ayah_embedding").all();
  if (rows.length) {
    // متّجهاتُ float32 صغيرةُ النهاية كما في مخطَّط القاعدة
    const vecs = rows.map((r) => {
      const b = r.vector;
      const buf = b.buffer ? Buffer.from(b.buffer, b.byteOffset, b.byteLength) : Buffer.from(b);
      return { id: r.ayah_id, v: new Float32Array(buf.buffer, buf.byteOffset, buf.byteLength / 4) };
    });
    neighborsOf = (id) => {
      const me = vecs.find((x) => x.id === id);
      if (!me) return [];
      const scored = [];
      for (const o of vecs) {
        if (o.id === id) continue;
        let d = 0;
        for (let i = 0; i < me.v.length; i++) d += me.v[i] * o.v[i];
        scored.push([o.id, d]);
      }
      scored.sort((a, b) => b[1] - a[1]);
      return scored.slice(0, 4).map(([i]) => i);
    };
  }
} catch { /* لا تضمينات — تُترك الجاراتُ فارغةً ولا يُختلق شيء */ }
