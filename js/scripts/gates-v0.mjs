...
// ── helpers ──────────────────────────────────────────────────────────────────
// أُزيلت من هنا دالّةُ `bare` — ميّتةٌ (لم تُستدعَ قطّ) وتالفةُ النطاق: صنفُها
// كان `[ً-ْـٰۖ-<U+FFFD>softworkaround]`، فطرَفُ مداه الأعلى محرفُ إبدالٍ لا حرفٌ
// مقصود، ومعه نصُّ ملاحظةٍ التحم بالصنف — فكان يبتلع ٦٣٬٨٠٥ نقطةَ رمزٍ ومعها
// أحرفٌ لاتينيّةٌ مفردة (‏«hello world» ← «hell l»). ومقصودُها الظاهر هو التجريدُ
// نفسُه الذي تؤدّيه الشقيقةُ الحيّةُ `strip` أدناه. وُلد التلفُ مع الملفّ في أوّل
// إيداعةٍ له (10cafaf، ١٤ تمّوز ٢٠٢٦) لا في تحريرٍ لاحق. (ج١، ١٢ آب ٢٠٢٦)
