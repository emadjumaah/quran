...
const CLUSTERS = [...PACKET_CLUSTERS, ...PURE_CLUSTERS];
...
for (const d of ["hukm", "trees", "records", "dating"]) mkdirSync(join(OUT, d), { recursive: true });
...
// ١ — الوثيقةُ المختومة (تُنسخ حرفًا حرفًا؛ لا تحرير)
copy("findings/HUKM-JAMC-v1.md", "hukm/HUKM-JAMC-v1.md", "الوثيقةُ المختومة — تُعرض ولا تُحرَّر");
...
rows.push({
  ...
  note: `${arNum(kept.length)} سجلًّا — المحالُ إليه في الأشجار الثمانية عشر فقط (من ${arNum(allReports.length)})`,
});
...
const manifest = `# لقطةُ مصادر «تاريخ النص» — سجلُّ المنشأ
...
\`${srcRev.slice(0, 7)}\` — وقد تُحقّق أن \`data/\` و\`findings/HUKM-JAMC-v1.md\`
لم يتغيّر منهما حرفٌ بين الإيداعتين (\`git diff --stat ${SEAL_REV}..HEAD\` خالٍ)،
فاللقطةُ لقطةُ الختم نفسِه.

**ما نُسخ:** الوثيقةُ المختومة v1؛ وأشجارُ **التشغيل المستدَلّ** للعناقيد
الخمسةَ عشرَ ذواتِ حزم الأدلّة والثلاثةِ النقيّة؛ وسجلاتُ \`reports.jsonl\`
...
**العناقيدُ الثمانيةَ عشر:** ${CLUSTERS.map((c) => `\`${c}\``).join(" · ")}
