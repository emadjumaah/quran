...
const P = {
  ...
  ids: join(VIEWS, "lib", "tarikhIds.ts"),
...
const PLANTS = [
  ...
  // ——— تأنيسُ العبارة المُعلَنة (س١٠): الخريطةُ والمصدرُ كلاهما مَحرَس ———
  ["خريطةُ التأنيس زوجٌ غيرُ مأذونٍ به", "ids",
    (s) => s.replace('["— تحقق خ٦/ب", "— تحقّق آليّ في مستودع البحث"],',
      '["— تحقق خ٦/ب", "— تحقّق آليّ في مستودع البحث"],\n  ["مصاحف الصحابة", "مصاحفُ الصحابة الكرام"],')],
  ["نصُّ التأنيس مُبدَّلٌ عمّا أُذن به", "ids",
    (s) => s.replace('"— تحقّق آليّ في مستودع البحث"', '"— تحقّق آليّ لا شكّ فيه"')],
  ["خريطةُ التأنيس مُعلَنةٌ ومعطَّلة", "ids",
    (s) => s.replace("PHRASE_MAP.reduce((s, [from, to]) => s.split(from).join(to), text)", "text")],
  ["التأنيسُ يتسرّب إلى البيانات (والمصدرُ لا يُمَسّ)", "claims",
    (s) => s.replace("صفر — تحقق خ٦/ب", "صفر — تحقّق آليّ في مستودع البحث")],
  ["التأنيسُ يتسرّب إلى مصدر العرض", "doc",
    (s) => s.replace("صفر — تحقق خ٦/ب", "صفر — تحقّق آليّ في مستودع البحث")],
];
