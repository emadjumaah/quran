...
/**
 * **النموذجُ المختار — من مستودعنا، وبأرقامه لا بالرغبة.**
 *
 * أصلُه `openai/whisper-tiny` مضبوطًا على تلاوة القرآن
 * (`tarteel-ai/whisper-tiny-ar-quran` — **Apache-2.0 معلَنةٌ في مصدرها**)،
 * **وصورتُه ONNX صدَّرناها نحن من تلك الأوزان بأعيانها** ثمّ كمّمناها بثمانِ
 * بتّات، **وموضعُها مستودعُنا** لا مستودعُ غيرِنا. وعُدّةُ التصدير مُودَعةٌ
 * (`tools/export_sawt_model.py`)، وتفصيلُه ورخصتُه في `CREDITS.md` §١٠
 * و`findings/S6-RESULTS.md`.
 *
 * **ولِمَ صُدِّر عندنا؟** لعلّتين مقيستين (ص-م٣ §٣ و§٤): مستودعُ التصدير
 * الثالثُ **لا يعلن رخصةً**، وصورتُه تحمل جدولَ الرموز **بدقّةٍ كاملةٍ ثمانين
 * ميغابايتًا** — فيثقل التنزيلُ **وتُغرَق ذاكرةُ سفاري على الهاتف**. وتصديرُنا
 * يفكّ ازدواجَ ذلك الجدول ويكمّمه، **فينزل المشحونُ من ٩٨ م.ب إلى ٤٣**.
 *
 * **والصورةُ `int8` لا `q4`** — بالقياس: التكميمُ إلى أربع بتّاتٍ يمسّ طبقةَ
 * الإخراج فيهبط التتبّع (٧٥٫٥٪ في الصفحة المتّصلة مقابل ٨٢٫٣٪ بثمانِ بتّات).
 *
 * **وكلُّ معرِّفٍ ورقمٍ ههنا يُقرأ من `model.json`** — وهو مانيفستُ ما يُنزَّل
 * فعلًا، مقيسًا بالبايت وتحرسه بوّابةُ المحرّك. **فلا رقمَ مكتوبٌ بيدٍ يتباعد
 * عمّا يُشحن.**
 */
export const MODEL_ID = MODEL.repo;
export const MODEL_DTYPE = MODEL.dtype;
/** سقفُ حجم النموذج المشحون المعلَن (م.ب) — تحرسه البوّابةُ برقمه */
export const MODEL_CAP_MB = MODEL.capMB;
/** وما يُنزَّل فعلًا — مجموعُ ملفّات المانيفست، لا تقديرًا */
export const MODEL_WIRE_MB = MODEL.totalMB;
...
export class OnDeviceRecognizer implements RecognizerPort {
   ...
   *   **التخزين** — أيسع الخزّانُ نموذجًا ٤٣ م.ب أم يمنعه حدُّ مساحة؟
  ...
  private async spawn() {
    ...
    try {
      ...
      // **ولا يبقى تنزيلٌ يجري بعد إخفاقٍ معلن**: كان العاملُ يُترك يُنزّل عشراتِ الميغابايتات
