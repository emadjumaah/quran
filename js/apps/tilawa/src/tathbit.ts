...
/**
 * **قيدُ الاصطياد في السجلّ — بها تكتمل الحلقة** (ن٢ §١): تسميعٌ ⇐ اصطيادٌ ⇐
 * سجلٌّ ⇐ جدولٌ يعيد الزوجَ في التدريب.
 *
 * **وبمفتاح الزوج نفسِه** الذي يُسأل به ههنا — فما اصطيد في التسميع يقع في
 * السطر الذي يُدرَّب عليه، لا في سطرٍ ثانٍ يوازيه. **وحكمُه حكمُ الخطأ في
 * الجدول** (`AGAIN`) فيُقرَّب أجلُه، **وعدُّ الخلط يزيد واحدًا** — وهو العدُّ
 * الذي يُعرض للحافظ، لا `lapses` الجدول.
 *
 * وهي دالّةٌ تُدعى من خارج الورقة (سطحُ التتبّع عند الختام)، **فتقرأ التخزينَ
 * وتكتبه** ولا تمرّ بحالٍ في ذاكرة الورقة — إذ قد تكون الورقةُ مغلقةً أصلًا.
 */
export function noteSlips(keys: string[]): void {
  if (!keys.length) return;
  const s = readStore();
  const now = Date.now();
  for (const k of keys) {
    const was = s[k] ?? null;
    s[k] = { ...review(was, AGAIN, now), misses: (was?.misses ?? 0) + 1 };
  }
  writeStore(s);
}
