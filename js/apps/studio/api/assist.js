...
export default async function handler(req) {
  ...
  if (body?.finalize) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      contents.push({ role: "user", parts: [{ text: attempt === 0 ? `${NUDGE}\nالموضع المعني: ${fviol}\nأعد كتابة الجواب كاملًا الآن (الأدوات مقفلة): كل اقتباسٍ ورقمٍ من النتائج أعلاه حرفيًّا، وما ليس فيها فاحذفه أو سمِّه بموضوعه.` : `ما زال في الجواب: ${fviol}. أعد الكتابة واحذفه حذفًا تامًّا.` }] });
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    contents.push({ role: "user", parts: [{ text: attempt === 0 ? `${NUDGE}\nالموضع المعني: ${viol}` : `ما زال في الجواب دعوى بلا سند: ${viol}. احذفها حذفًا تامًّا من الجواب (لا تعوضها من ذاكرتك) أو استدعِ أداةً تأتي بسندها.` }] });
