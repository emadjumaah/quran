...
function istinbatViolation(text) {
  ...
  // **ثم طبّع مرةً واحدةً، وافحص كلَّ شيءٍ على المطبَّع** (ج١، ١٢ آب ٢٠٢٦): كان
  // شرطُ الدخول وحدَه يفحص `squash(own)` وسائرُ الشروط تفحص الخام — فينكسر
  // التطابقُ على جوابٍ مشكول. وأثرُه وجهان لا وجهٌ واحد: إنذارٌ كاذبٌ على جوابٍ
  // سليمٍ مشكول («مُقَدِّمات»، «لَعَلَّ»)، **وإفلاتُ مخالفةٍ مشكولةٍ من أشدِّ ما
  // يُحرس** — «أرادَ اللهُ بذلك» كانت تمرّ سالمةً وهي عبارةُ قطعٍ في مراد الله.
  // ولذلك تُكتب المصطلحاتُ المفحوصةُ هنا **مجرَّدةً** كالنصِّ المفحوص سواء
  // (عبرةُ بلاغ 2026-08-12-arabic-word-boundary: المصطلحُ يمرّ بمُطبِّع نصِّه).
  const own = squash(text.replace(/﴿[^﴾]*﴾/g, " ").replace(/«[^»]*»/g, " "));
  if (/أراد الله بذلك|مراد الله (?:قطع|يقين|هو )|الحكمة الإلهية (?:من|في) ذلك هي|قصد الله بهذا/.test(own)) {
    return "عبارةُ قطعٍ في مراد الله — الاستنباط يفسر النمط المحسوب بصيغة الاحتمال ولا يتكلم في مراد الله";
  }
  if (!own.includes("استنباط مولد")) return null;
  // ترقيمُ المقدمات على نصٍّ مجرَّد: لا تشكيلَ فيه أصلًا، فبقي من الفاصل ما يقع فعلًا
  if (!/مقدمات/.test(own) || !/[١1][).]?[\s)]/.test(own)) return "استنباطٌ مولَّد بلا مقدماتٍ مرقمةٍ معروضة — اعرض مقدماته: ١) … ٢) …";
  if (!/(لعل|يحتمل|يظهر أن|كأن|قد يكون|ربما)/.test(own)) return "استنباطٌ بلا صيغة احتمال — صُغ الاستنتاج بـ«لعل» ونحوها";
  if (!/(حدود|يضعف|لو ظهر|يبقى احتمال)/.test(own)) return "استنباطٌ بلا جملةِ حدوده — اختمه بما لو ظهر لأضعفه";
  return null;
}
...
export default async function handler(req) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    // الحراسُ الثانوية تُفحص على المطبَّع بالمعيار نفسِه (ج١)، ومصطلحاتُها
    // مكتوبةٌ مجرَّدةً: «خطةٍ بحثية» صارت «خطة بحثية» — ولولا ذلك لماتت بالتطبيع.
    const sqText = squash(text ?? "");
    const promises = /سأبحث|سنبحث|نبدأ بها|خطة بحثي|خطة بحثية|يمكننا أن نبحث|هل نبدأ|هل تريد أن أبدأ|هل هذه الخطة|لنبدأ|سأقترح عليك خطة/.test(sqText);
    ...
    // `organizing` **كاسرٌ** للحارس لا حارس، فيُتوقّى توسيعُه: «رتّب» بعد التطبيع
    // تصير «رتب»، ولو تُركت مفردةً لكسرت الحارسَ عند كلِّ «رتبة/مرتب» عارضة.
    // فبقي منها المقيَّدُ «رتب لي» — والمشكولةُ «رتّب لي» صارت تصيبه بعد التطبيع
    // وكانت تخطئه، و«رتّب» المفردةُ يغنيها ما بعدها من الألفاظ («محاور» ونحوها).
    const organizing = /رتب لي|محاور|خطة البحث|ناقشني|لا تكتب/.test(squash(lastUser));
    const knowledgeish = /ورد في القرآن|في القرآن الكريم|الجذر|المعنى اللغوي|دلالات|يدل على|معناه|استعمالات|السياق القرآني/.test(sqText);
    ...
    const denial = text && steps.length === 0 && /لا نقل عندي|لم أجد في بياناتي|لا تكفي المقدمات|ليس عندي في هذا/.test(sqText)
