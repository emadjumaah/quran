...
/** لحظةُ إقلاع هذه الجلسة — وبها وحدَها يُفرَّق الرجوعُ بعد انصرافٍ من الجولان
  ...
  /* **والعودةُ بلا استجواب** (ج٤ §٣/٤): يُفتح فيجد نفسَه حيث وقف — **ولا سؤالَ
     ...
     **فأدقُّ شرطٍ صادقٍ اثنان معًا**:
       ١ — أن يكون الموضعُ **محفوظًا فعلًا** بختمٍ زمنيّ (`at`) — فميراثُ ما قبل
           طبقة المواضع يُقرأ بلا `at`، ولا يُدرى أرجوعٌ هو أم لا، فلا يُدَّعى؛
       ٢ — وأن يكون ختمُه **قبل إقلاع هذه الجلسة** — فما كُتب في الجلسة نفسِها
           جولانٌ في التطبيق لا رجوعٌ إليه.
     **ولا يُمَسّ الانتقالُ نفسُه**: يُفتح على موضعه في الحالين، وإنّما يُمنع
     الادّعاءُ وحدَه. */
  const saved = m?.at ? Date.parse(m.at) : NaN;
  const returning = Number.isFinite(saved) && saved < SESSION_START;
  return at ? (
    <Navigate
      to={`/read/${at.surahNo}/${at.ayahNo}`}
      replace
      state={returning ? { resumed: `${at.surahNo}:${at.ayahNo}` } : undefined}
    />
  ) : (
